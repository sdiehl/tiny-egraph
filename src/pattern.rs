...
impl Pattern {
    ...
    /// # Panics
    /// Panics if the pattern has no nodes.
    #[must_use]
    pub fn root(&self) -> Id {
...
pub fn instantiate(pat: &Pattern, subst: &Subst, egraph: &mut EGraph) -> Id {
    fn go(
        pat: &Pattern,
        id: Id,
        subst: &Subst,
        ...
        let new_id = match pat.get(id) {
            PatternNode::Var(v) => subst
                .get(v)
                .unwrap_or_else(|| panic!("unbound variable {v} during instantiate")),
            PatternNode::Op { op, children } => {
